						...
						char *		outDesc)
{
	...
	if (file_data)
	{
		...
		fprintf(file_data,"Hr\t");
		fprintf(file_data,"Hg\t");
		fprintf(file_data,"dH\t");
	...
	void *               inRefcon)
{
	...
	{
		flag_sit = 0;
		XPLMLoadDataFile(xplm_DataFile_Situation, "Output\\situations\\8_Hg.sit");
		F2_on = 1;
	}
	if (F2_on)
	{
...
// 		float f = fff[0];
		...
		if (!flag_start && vect_sost.time>50)
		{
			flag_start = 1;
		...
		if (!flag_stop && (vect_sost.time-start_time)>150) 
		{
			flag_stop = 1;
		...
		if (1)//крен
		{
			...
			float kren_zad = 0;
			static float kurs_zad = 90;//vect_sost.kurs;
			float kurs = vect_sost.kurs;
			if (kurs>180.)
				kurs = kurs - 360 ;

			//kren_zad = -1*(kurs-90);
			//Limit(kren_zad,30);
			if (vect_sost.Vpr>150 && start_time)
			{
				if (bbb)
				{
					float new_kurs_zad = kurs_zad+45;
					if (new_kurs_zad>180.)
						new_kurs_zad = new_kurs_zad - 360 ;
					if (new_kurs_zad<-180.)
						new_kurs_zad = new_kurs_zad +360 ;
					kren_zad = -1*(kurs-new_kurs_zad);
					Limit(kren_zad,30);
				} 
				else
				{
			...
			//if (flag_start)
			//{
			//	if ((vect_sost.time-start_time)>10 && (vect_sost.time-start_time)<=25)
			//		kren_zad = -40;
			//	else if ((vect_sost.time-start_time)>25 && (vect_sost.time-start_time)<=40)
			//		kren_zad = 40;
			//	else if ((vect_sost.time-start_time)>40)
			//		kren_zad = 0;
			//}
			...
			//kren_zad = -30;
				...
				//kren_zad = 45*sin(vect_sost.time*2*M_PI*0.1);
			float dx = 1.5*(kren_zad - vect_sost.kren) - 0.5*vect_sost.wx;
			//float dx = 0.1*vect_sost.wx;
			...
			if (1)//танг
			{
				...
				if (1)
				{
					float T = 0.5;
					static float y2;
					float x = vect_sost.wz;
					y2+= (1/T*(x-y2))*vect_sost.dt;
					float y = x- y2;
					izod_wz = y;				
				}

				tang_zad = 0.5*(vect_sost.Vpr-220);
		...
		//{
			...
			//static float intH;
			//if (fabs(Hzad-Htek)<100)
			//	intH+=0.05*raznH*vect_sost.dt;
			//Limit(intH,400);
			...
			if (file_data && flag_start && !flag_stop)
			{
				...
				fprintf(file_data,"%f\t", vect_sost.Hr+Hzad);
				fprintf(file_data,"%f\t", vect_sost.Hr);
				fprintf(file_data,"%f\t", vect_sost.Hr +vect_sost.Hg);
				fprintf(file_data,"%f\t", vect_sost.Hg-Hzad);
				fprintf(file_data,"%f\t", vect_upr.osh);
				fprintf(file_data,"%f\t", vect_sost.kren);
				fprintf(file_data,"%f\t", vect_sost.tang);
				fprintf(file_data,"\n");

				//fprintf(file_data,"%f\t%f\t\n", vect_sost.time, ,vect_sost.Hb);
			}
	}
	return 1;
