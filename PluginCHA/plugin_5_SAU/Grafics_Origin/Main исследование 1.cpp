	...
	void *               inRefcon)
{
	...
	{
		flag_sit = 0;
		XPLMLoadDataFile(xplm_DataFile_Situation, "Output\\situations\\6_500.sit");
		F2_on = 1;
	}
	if (F2_on)
	{
...
// 		float f = fff[0];
		...
		if (!flag_start && vect_sost.time>25)
		{
			flag_start = 1;
		...
		if (!flag_stop && (vect_sost.time-start_time)>50) 
		{
			flag_stop = 1;
		...
		if (1)//крен
		{
			...
			float kren_zad = 0;
			//if (vect_sost.Vpr>150 && vect_sost.time>30)
			//{
			//	if (bbb)
			//	{
			//		kren_zad = 50;
			...
			float dx = 0.4*(kren_zad - vect_sost.kren) + 0.1*vect_sost.wx;
			...
			if (1)//танг
			{
				...
				float dz = 1.0*(tang_zad - vect_sost.tang) + 0.4*vect_sost.wz;
		...
		//{
			...
			if (file_data && flag_start && !flag_stop)
			{
				...
				fprintf(file_data,"%f\t", RmnkVar.e_id/2.0*100);//max dVy = 2.0 //100%
