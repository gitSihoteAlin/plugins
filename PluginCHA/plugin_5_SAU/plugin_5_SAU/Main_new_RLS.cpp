...
// 	}
	...
	if (file_data && flag_start && !flag_stop)
	{
		...
		fprintf(file_data,"%f\t", vect_sost.Vy);
		fprintf(file_data,"%f\t", vect_sost.Hb);
		fprintf(file_data,"%f\t", vect_sost.Hb-vect_sost.Hg_rv);
		fprintf(file_data,"%f\t", vect_upr.osh);
		fprintf(file_data,"%f\t", vect_upr.Vymax);
		fprintf(file_data,"%f\t", vect_sost.Hg_rv);
		fprintf(file_data,"%f\t", vect_sost.Hg);
		fprintf(file_data,"%f\t", vect_sost.Hzad);
		fprintf(file_data,"%f\t", vect_sost.Hg-vect_sost.Hzad);
		fprintf(file_data,"\n");
	}
	...
	if (1)
	{
		...
		{
			flag_up_down_beam = 0;
			memset(&p[c], 0, sizeof(Tpoint)*(N_DATA_RLS-c));
			c = 0;
			vect_upr.VyRLS = VymaxRLS;
			VymaxRLS = -10.f;
		}
		...
		if (vect_sost.D_naklon<MAX_Dist_RLS && c<N_DATA_RLS)
		{
			
			p[c].L = vect_sost.D_naklon;
			...
			p[c].Htr = vect_sost.Hg_rv+ p[c].L*sin(p[c].alfa)+Hzad_RLS;

			float T = 1.f;//Т=1 сек – постоянная времени, характеризующая динамику ЛА;
			p[c].Vytr = (p[c].Htr - vect_sost.Hg_rv - vect_sost.Vy*T)/(p[c].t - T);

			if (p[c].Vytr>VymaxRLS) VymaxRLS = p[c].Vytr;
		...
		if (vect_upr.VyRLS>10.f) vect_upr.VyRLS = 10;
	}
	else
		vect_upr.VyRLS = 0;
}
...
void GetVectorSost()
{
	...
	vect_sost.Hr = vect_sost.Hb - vect_sost.Hg_rv;
	...
	void *               inRefcon)
{
	...
	IniSituation("Output\\situations\\7.sit");
...
void Paint_all()
{
	...
	glPushMatrix();
	{
		...
		H_str = 350;
		i = 0;
		glRasterPos2f(X_str, H_str-d_Hstr*i++);	glPrint("Hg = %4.0f ", vect_sost.Hg_rv);
                                   ...
                                   void *               inRefcon)
{
	...
	if (1)//F1_on)
	{
		...
		//Vy	 = XPLMGetDataf(gPlane_local_vy);
		...
		glPushMatrix();
			...
			glPrint("Hrv = %3.0f m", vect_sost.Hg_rv);
			...
			if (F6_on)
			{
				...
				glPrint("D_naklon = %5.0f", vect_sost.D_naklon);
			}
	...
	* guarantees that our axes will be seen no matter what. */
	...
	if (F5_on)
	{
		...
		for (int i2 = 0; i2<N_dots; i2++)
		{
			...
			Rel[i2].x = planeX+sin(kurs_rad)*dL*i2;
			Rel[i2].z = planeZ-cos(kurs_rad)*dL*i2;
			...
			Tg_e[i2]=(Rel[i2].y-(Rel[0].y+vect_sost.Hg_rv))/double(dL*i2); //tangensi uglov vozvyshenia
			if(Tg_e[i2]<=td)Tg_e[i2] = td;     //bylo -maxdbl
				else td=Tg_e[i2];
		}
		...
		if (F6_on)
		{
			...
			//формирование координат x и z рельефа (в которых будем искать высоту рельефа)
			float kurs_rad = (vect_sost.kurs+angle_azimuth_RLS)*M_PI/180.f;
			line_beam.x = planeX+sin(kurs_rad)*MAX_Dist_RLS;
			line_beam.z = planeZ-cos(kurs_rad)*MAX_Dist_RLS;
			line_beam.y = planeY+tan(eps)*MAX_Dist_RLS;
		...
		FloatVals[8] = vect_sost.D_naklon ;
